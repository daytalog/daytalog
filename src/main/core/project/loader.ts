...
import {
  ...
  ProjectMenuItem
} from '@shared/core/project-types'
...
const parseSettingsFile = async <T>(filePath: string, schema: ZodType<T>): Promise<T | null> => {
  if (fs.existsSync(filePath)) {
    ...
    if (parse.success) {
      ...
      throw new Error(`${z.prettifyError(parse.error)}`)
    }
  } else {
...
export const getTemplateDirectories = (
  ...
  const projectPath = _projectPath ?? appState.config.activeProject
...
export const loadProject = async (): Promise<
  { success: true; data: ProjectRootType } | { success: false; error: string }
> => {
  logger.debug('loadProject started')
  const projectPath = appState.config.activeProject
  if (!projectPath) throw new Error('No active project path')
  ...
  const projectFilePath = path.join(projectPath, 'config.yaml')
  ...
  try {
    ...
    if (projectSettings.project_name !== path.basename(projectPath)) {
      await updateProjectFolder(projectSettings.project_name)
    }
    const templatesDir = await loadTemplates(projectPath)
    const activeLog = appState.config.activeLog
    ...
    logger.error(error)
    console.error(error)
    const errorMessage = 'An error occured while loading settings'
