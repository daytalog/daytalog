...
import { getEditorWindow } from '@core-windows/editor/editorWindow'
...
const updateTemplatesDir = (filePath: string, action: 'add' | 'remove') => {
  ...
  if (action === 'add') {
    ...
    const deduped = [...templatesDir, newTemplate].reduce<TemplateDirectoryFile[]>((acc, cur) => {
      if (!acc.some((t) => t.path === cur.path)) acc.push(cur)
      return acc
    }, [])
    activeProject.templatesDir = deduped
