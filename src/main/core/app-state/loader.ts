...
async function loadStateFromFile(filepath: string): Promise<state> {
  ...
  try {
    ...
    //const decrypted = safeStorage.decryptString(fileData) // Removed safe storage, we don't store any secrets anyway.
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        await updateState({ activeProject: null, activeLog: null })
...
export async function loadState(): Promise<void> {
  ...
  if (appState.config.activeProject) {
    const loadActiveProject = await loadProject()
...
async function ensureConfigExist(config: string) {
  ...
  if (!exists) {
    ...
    await ensureDirectoryExists(path.dirname(config))
    const defaultYaml = { logid_template: 'D<dd>_<yymmdd>', version: 1 }
