...
export function setupEditorIpcHandlers(): void {
  ...
  ipcMain.handle('initial-editor-data', async (): Promise<InitialEditorData> => {
    try {
      const loadedDaytalogs = Array.from(daytalogs().values())
      const { config, localSharedPath, project } = appState

      if (!project || !config.activeProject) throw Error('No active project')
