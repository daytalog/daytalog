...
import { appState, daytalogs as daytalogStore, sendWindowDataMap } from '../../app-state/state'
...
export function setupSendIpcHandlers(): void {
  ipcMain.handle('initial-send-data', async (event): Promise<InitialSendData> => {
    try {
      ...
      if (!project) throw new Error('No active project in appState')
    ...
    async (event, email: EmailType): Promise<Response> => {
      ...
      try {
        ...
        const successotification = new Notification({
          title: 'Email Sent',
          body: `"${email.subject}" sent successfully to ${email.recipients.length} recipient${email.recipients.length > 1 ? 's' : ''}`,
