...
const sendSchema = z.object({
  ...
  logId: z.string().nonempty().or(z.array(z.string()))
})
...
const getSelectedLogs = (selection: string | string[]): LogType | LogType[] | undefined => {
  if (typeof selection === 'string') {
    return daytalogs().get(`${appState.config.activeProject}/logs/${selection}.dayta`)
  } else if (Array.isArray(selection)) {
    const items = selection
      .map((id) => daytalogs().get(`${appState.config.activeProject}/logs/${id}.dayta`))
      .filter((d): d is LogType => d !== undefined)
...
export async function startLocalServer() {
  ...
  // List Logs
  api.get('/logs', (_req: Request, res: Response) =>
    res.json({ daytalogs: [...daytalogs().values()] })
  )

  // Create log
  api.post('/log', async (req: Request, res: Response) => {
    const project = appState.project
    if (!project) {
      return res.status(404).json({ error: 'No project loaded' })
    }

    const parseResult = LogDynamicZod(project).safeParse(req.body)
    if (!parseResult.success) {
      return res.status(400).json({ error: z.prettifyError(parseResult.error) })
    }
    const response = await updateDaytalog(parseResult.data as LogType, null)
    if (!response.success) {
  ...
  // Get Log
  api.get('/log/:id', (req: Request, res: Response) => {
    const projectpath = appState.config.activeLog
    if (!projectpath) {
      return res.status(404).json({ error: 'No project loaded' })
    }
    ...
    const log = daytalogs().get(`${projectpath}/logs/${id}.dayta`)
    if (!log) {
      return res.status(404).json({ error: 'Requested log not found' })
    }
    return res.status(200).json(log)
  })

  // Update Log
  api.patch('/log/:id', async (req: Request, res: Response) => {
    ...
    const path = `${projectPath}/logs/${id}.dayta`
    const existing = daytalogs().get(path)
    if (!existing) {
      return res.status(404).json({ error: 'Log not found' })
    }

    const parseResult = LogDynamicZod(project).partial().safeParse(req.body)
    if (!parseResult.success) {
      return res.status(400).json(formatErrors(parseResult.error))
    }

    const updatedLog = { ...existing, ...parseResult.data } as LogType

    const updateResponse = await updateDaytalog(updatedLog, null, existing)
  ...
  // Delete Log
  api.delete('/log/:id', async (req: Request, res: Response) => {
    const idParse = idSchema.safeParse({ id: req.params.id })
    if (!idParse.success) {
      return res.status(400).json(formatErrors(idParse.error))
    }
    ...
    const path = `${projectPath}/logs/${id}.dayta`
    const log = daytalogs().get(path)
    if (!log) {
      return res.status(404).json({ error: 'Log not found' })
    }

    const deletion = await deleteDaytalog(log)
    if (!deletion.success) {
  ...
  api.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      const { presetId, logId } = parseResult.data
      const email = appState.project?.emails?.find((email) => email.id === presetId)
      const selectionCheck = getSelectedLogs(logId)
      if (!selectionCheck) {
        return res.status(404).json({ error: 'Could not find logs from IDs' })
      }
      const selection = typeof logId === 'string' ? [logId] : logId
      try {
