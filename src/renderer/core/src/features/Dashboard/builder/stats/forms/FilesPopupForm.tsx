...
export const FilesPopupForm: React.FC<FilesPopupFormProps> = ({
  ...
  const form = useForm<fileFormType>({
    ...
    mode: 'onSubmit',
    resolver: zodResolver(fileSchema) as Resolver<fileFormType>
  })
  ...
  useEffect(() => {
    reset({
      files: value?.files ?? 0,
      size: value?.size ? format.formatBytes(value.size, { output: 'number', type: 'gb' }) : 0,
      sizeUnit: 'gb',
      copies: value?.copies ?? []
    })
  }, [value])
  ...
  const onSubmit: SubmitHandler<fileFormType> = (data): void => {
    if (!value) return
    const sizeInBytes = data.size ? format.convertToBytes(data.size, data.sizeUnit) : 0
