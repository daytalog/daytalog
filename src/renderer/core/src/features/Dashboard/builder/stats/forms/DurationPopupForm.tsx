...
import { DurationType } from '@shared/core/shared-types'
...
export const DurationPopupForm: React.FC<DurationPopupFormProps> = ({
  ...
  const form = useForm<DurationType>({
    ...
    mode: 'onSubmit',
    resolver: zodResolver(durationSchema) as Resolver<DurationType>
  })
  const {
    handleSubmit,
    reset,
  ...
  const onSubmit: SubmitHandler<DurationType> = (data): void => {
    if (data !== value) {
      const duration = format.formatDurationToTC(data)
      update(duration)
    }
    setIsOpen(false)
  }
