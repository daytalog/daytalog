import type { OcfClipType, SoundClipType, ProxyClipType, CustomClipType } from 'daytalog'
...
export type CustomClipTypeExtended = CustomClipType & {
  id: string
  schemaSync: 'tc' | 'clip'
  schemaIndex: number
  clipIndex: number
  ...
  mergeIntoMap(clipMap, ocfClips, (merged, ocfClip) => {
    ...
    for (let i = 0; i < maxCopies; i++) {
      if (Array.isArray(ocfClip.copies) && ocfClip.copies[i]) {
        ...
        const cleanHash = copy.hash ?? 'No Hash'
        if (!merged['hash'].value.includes(cleanHash)) {
          merged['hash'].value.push(cleanHash)
        }
      } else {
        merged[`copy_${i + 1}`] = {
          path: `ocf.clips[${ocfClip.index}]`,
...
function mergeSoundClips(
  ...
  const { timecodeToSeconds, rangesOverlap } = format
...
function mergeCustomEntries(
  ...
  // Narrow so we only pass items with a defined string `clip` into mergeIntoMap.
  const isClipSyncClip = (
    c: CustomClipTypeExtended
  ): c is CustomClipTypeExtended & { clip: string } =>
    c.schemaSync === 'clip' && typeof c.clip === 'string'
  const isTcSyncClip = (
    c: CustomClipTypeExtended
  ): c is CustomClipTypeExtended & { clip: string } =>
    c.schemaSync === 'tc' && typeof c.clip === 'string'
  const clipClips = customClips.filter(isClipSyncClip)
  const tcClips = customClips.filter(isTcSyncClip)
  const baseExcludedProps = ['index', 'id', 'schemaSync', 'schemaIndex', 'clipIndex']
  const excludedForClip = new Set([...baseExcludedProps, 'clip'])
  const excludedForTc = new Set([...baseExcludedProps, 'tc_start', 'tc_end'])

  if (clipClips.length) {
    mergeIntoMap(clipMap, clipClips, (merged, customClip) =>
      mergeCustom(merged, customClip, excludedForClip)
    )
  }
  if (tcClips.length) {
    const { timecodeToSeconds, rangesOverlap } = format
    for (const [, mergedClip] of clipMap) {
      ...
      for (const cClip of tcClips) {
        if (!cClip.tc_start || !cClip.tc_end) {
          continue
        }
        const customStart = timecodeToSeconds(cClip.tc_start)
        const customEnd = timecodeToSeconds(cClip.tc_end)

        if (rangesOverlap(ocfStartFrames, ocfEndFrames, customStart, customEnd)) {
          mergeCustom(mergedClip, cClip, excludedForTc)
        }
      }
    }
  }
...
function mergeCustom(
  ...
  customClip: CustomClipTypeExtended,
  excludedProps: Set<string>
) {
  Object.keys(customClip).forEach((prop) => {
    if (excludedProps.has(prop)) return
    const value = customClip[prop]
    const basePath = `custom[${customClip.schemaIndex}].clips[${customClip.clipIndex}]${typeof value === 'object' && value !== null ? `.${prop}` : ''}`
