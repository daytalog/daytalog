        ...
        if ((colId === 'size' || colId === 'proxy_size') && typeof value.value === 'number') {
          return (
            <span className="whitespace-nowrap">
              {format.formatBytes(value.value, { output: 'string' })}
            </span>
          )
        }
        ...
        {table.getHeaderGroups().map((headerGroup) => (
          <TableRow key={headerGroup.id}>
            {headerGroup.headers.map((header) => (
              <TableHead
                key={header.id}
                className="fixed top-0 border-b-0 first:sticky first:left-0 sticky top-0 z-20 bg-background first:z-20 first:bg-background first:shadow-[inset_-1px_0_0_0_hsl(240_3.7%_15.9%),inset_0_-1px_0_0_hsl(240_3.7%_15.9%)] shadow-[inset_0_-1px_0_0_hsl(240_3.7%_15.9%)] capitalize"
              >
