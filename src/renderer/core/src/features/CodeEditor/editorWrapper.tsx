...
import { LoadedFile } from '@shared/core/shared-types'
...
const EditorWrapper = () => {
  ...
  const [mockdata, setMockdata] = useState<mockDataType>({
    ...
    source: initialData.loadedDaytalogs.length ? 'logs' : 'generated',
    cacheLog: initialData.loadedDaytalogs.length
      ? initialData.loadedDaytalogs.map((l) => l.id).join()
      : generatedDaytalogs.map((l) => l.id).join(),
    logs: initialData.loadedDaytalogs.length ? initialData.loadedDaytalogs : generatedDaytalogs
  })
  ...
  return (
    <ResizablePanelGroup direction="horizontal" className="w-full border-l">
      ...
      <ResizablePanel className="flex flex-col p-2 gap-2 bg-[#323639]">
          ...
          <div className="flex gap-1">
            <NewMockdataDialog mockdata={mockdata} setMockdata={setMockdata}>
              <Button
                size="sm"
                variant="outline"
                className="rounded z-40 bg-transparent"
                style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}
              >
            ...
            <Button
              size="sm"
              variant="outline"
              className="z-40 rounded text-xs bg-transparent"
              style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}
            ...
            <Button
              ...
              style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}
              onClick={() =>
                window.externalApi.openExternal('https://daytalog.com/pricing#service')
              }
            >
              Design Help
            </Button>
            ...
            >
              ...
              >
                <Tooltip>
                  ...
                  <TooltipContent>Single selection</TooltipContent>
                </Tooltip>
              </ToggleGroupItem>
              <ToggleGroupItem
                value="multi"
              ...
              >
                <Tooltip>
                  ...
                  <TooltipContent>Multiple selection</TooltipContent>
                </Tooltip>
              </ToggleGroupItem>
            </ToggleGroup>
          </div>
        ...
        <ResizablePanelGroup direction="vertical">
          ...
          <ResizableHandle withHandle />
          <ConsolePanel currentFileId={selectedFile?.path} />
        </ResizablePanelGroup>
      </ResizablePanel>
    </ResizablePanelGroup>
  )
