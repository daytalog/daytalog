...
import { createTypeDefinition } from 'daytalog/typegen'
...
export async function loadTypeDefinitions(
  ...
  const reactTypeDefs = import.meta.glob(
  ...
  // React's CSSProperties depends on the 'csstype' package for all CSS property names
  const cssTypeDefs = import.meta.glob('../../../../../../../node_modules/csstype/**/*.d.ts', {
    query: '?raw',
    import: 'default',
    eager: true
  })

  const reactPdfTypeDefs = import.meta.glob(
    '../../../../../../../node_modules/@react-pdf/**/*.d.ts',
    {
      query: '?raw',
      import: 'default',
      eager: true
  ...
  const { Clip, OcfClip, ProxyClip, SoundClip, Custom } = createTypeDefinition(project)
  const merged = [
  ...
  const dtsModules = { ...reactTypeDefs, ...cssTypeDefs, ...reactPdfTypeDefs }
