...
import { LoadedFile, ChangedFile } from '@shared/core/shared-types'
...
const Editor = forwardRef<EditorHandle, EditorProps>((props, ref) => {
  ...
  useEffect(() => {
    const fetchAndSendSelection = async () => {
      ...
      const latestSelection =
        selection === 'multi'
          ? mockDataRef.current.logs.slice(-2).map((log) => log.id)
          : mockDataRef.current.logs.slice(-1).map((log) => log.id)
  ...
  useEffect(() => {
    ...
    previewWorker.onmessage = (e: MessageEvent<PreviewWorkerResponse>): void => {
      ...
      if (msg.msgtype === 'read-files-base64') {
        ...
        window.base64Api.readBase64Files(base, paths).then((data) => {
      ...
      if (msg.msgtype === 'preview-update') {
        console.debug('recieved preview-update')
        const previewEvent = new CustomEvent('preview-update', { detail: msg })
        window.dispatchEvent(previewEvent)
        return
      }
      if (msg.msgtype === 'preview-console') {
        console.debug('recieved preview-console')
        const consoleEvent = new CustomEvent('preview-console', { detail: msg })
    ...
    (model: monaco.editor.ITextModel, file: FileTrackingInfo): void => {
      try {
        ...
        const daytalogProps: DaytalogProps = {
          project,
          cacheLog: mockDataRef.current.cacheLog,
          logs: mockDataRef.current.logs,
  ...
  const handleContentChange = useCallback(
    (
      ...
      console.debug(model, file, event)
  ...
  useImperativeHandle(ref, () => ({
    openFile(file: LoadedFile) {
      ...
      if (!fileTrackers.has(uriKey)) {
        console.debug('new tracker is being set')
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.debug('Save operation successful. Updating trackers...')
      ...
      urisToUpdate.forEach((uriKey) => {
        ...
        console.debug(`Tracker updated for URI: ${uriKey}`)
      })

      // Step 5: Reset the dirty state
      resetDirty()
      console.debug('All dirty files saved successfully!')
    } catch (error) {
  ...
  // On file open, send code to worker immediately if workerReady is true
  useEffect(() => {
    ...
    sendMessageToWorker(model, file)
  }, [workerReady])
